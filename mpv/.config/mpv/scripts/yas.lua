...
local function get_user_stats()
    local response = http_request(endpoints.user_stats, "GET", {
        userID = options.user_id
    })
    if not response then
        mp.msg.warn("⚠️ [yas] Failed to get user stats")
        return
    end
    local data = utils.parse_json(response)
    if not data then
        mp.msg.warn("⚠️ [yas] Failed to parse user stats JSON")
        return
    end
...
if options.user_id then
    mp.add_key_binding("u", "show_user_stats", get_user_stats)
end
