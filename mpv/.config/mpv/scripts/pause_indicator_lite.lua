...
local function draw_rectangles()
	if options.fluent_icons then
		...
		return string.format(
			[[{\\rDefault\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&\\fs%s\\fn%s}%s]],
			icon_opacity,
			options.icon_border_width,
			icon_color,
			icon_border_color,
			options.fluent_icon_size,
			icon_font,
			pause_icon
		)
	else
		return string.format(
			...
			options.rectangles_width,
			options.rectangles_height,
			options.rectangles_height,
			options.rectangles_width + options.rectangles_spacing,
...
local function draw_triangle()
	if options.fluent_icons then
		...
		return string.format(
			[[{\\rDefault\\p1\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&}m 0 0 l %d %d l 0 %d{\\p0}]],
			icon_opacity,
			options.icon_border_width,
			icon_color,
			icon_border_color,
			options.triangle_width,
			options.triangle_height / 2,
...
local function update_indicator()
	...
	if display_aspect == 0 or (indicator.visible and not toggled) then
		return
	end
	...
	if not options.indicator_stay then
		mp.add_timeout(options.indicator_timeout, function()
			indicator:remove()
		end)
	end
end
...
local function flash_icon()
	...
	flash:update()
	mp.add_timeout(options.flash_icon_timeout, function()
		flash:remove()
	end)
end
...
mp.observe_property("pause", "bool", function(_, paused)
	...
	if paused then
		update_indicator()
		toggled = true
		if options.flash_play_icon then
			flash:remove()
		end
	else
		indicator:remove()
